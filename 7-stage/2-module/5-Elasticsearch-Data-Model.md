...
2. 逻辑建模阶段
   ...
   逻辑模式是对概念模式的进一步分解和细化，描述了实体、实体属性以及实体之间的关系，是概念模型延申，一般的逻辑模型有第三范式，星型模型和雪花模型。模型的主要元素为主题、实体、实体属性和关系。
...
```
举例：借助logstash实现mysql到Elasticsearch的增量同步，
如果数据建模阶段没有设计时间戳或者自增ID，就几乎无法实现。
```
