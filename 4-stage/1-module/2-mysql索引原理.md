...
- 一般性分页
  ...
  - 第一个参数指定第一行返回记录行的偏移量，注意从 0 开始；
  - 第二个参数指定返回记录行的最大数目；
  - 如果只给定一个参数，它表示返回最大的记录行数目；

  **思考一：如果偏移量固定，返回记录量对执行时间有什么影响？**
  ...
  结果：在查询记录时，返回记录量低于 100 条，查询时间基本没有变化，差距不大。随着查询记录量越大，所花费的时间也越来越多。
...
- 分页优化方案
  ...
  **第二步：利用子查询优化**
  ...
  原因：使用了 id 做主键比较（ id>= ），并且子查询使用了覆盖索引进行了优化。
