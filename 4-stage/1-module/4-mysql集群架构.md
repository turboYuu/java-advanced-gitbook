...
当系统用户进入了高速增长期，即便是对数据进行分库分表，但数据库的容量，还有表的数据量也总会达到天花板。当现有数据库达到承受极限时，就需要增加新服务器节点数量进行横向扩容。

横向扩展会有什么技术难度？

![image-20220919181340728](assest/image-20220919181340728.png)
...
数据库扩容的过程中，如果想要持续对外提供服务，保证服务的可用性，平滑扩容方案是最好的选择。平滑扩容就是将数据库扩容成原来的2倍，比如2个数据库扩容到4个数据库，具体步骤如下：

1. 新增2个数据库。

2. 配置双主进行数据同步（先测试，后上线）。

   ![image-20220919184840820](assest/image-20220919184840820.png)

3. 数据同步完成之后，配置双主双写（同步因为有延迟，如果实时都有写和更新操作，会存在不准确问题）。

   ![image-20220919184854495](assest/image-20220919184854495.png)

4. 数据同步完成后，删除双主同步，修改数据库配置，并重启；

   ![image-20220919184906109](assest/image-20220919184906109.png)

5. 此时已经扩容完成，但此时数据并没有减少，新增的数据库跟就得数据库一样多的数据，此时还需要写一个程序，清空数据库中多余的数据：
...
平滑扩容方案能够实现 n 库的平滑扩容，增加数据库服务能力，降低单库一般的数据量。其核心原理：成倍扩容，避免数据迁移。

优点：

- 扩容期间，服务正常进行，保证高可用。
- 相对停机扩容，时间长，项目组压力没有那么大，出错率低。
