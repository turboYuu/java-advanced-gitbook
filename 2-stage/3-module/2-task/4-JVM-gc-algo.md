...
4. **参数**
   ...
   - 最大垃圾收集停顿时间 -XX:MaxGCPauseMillis
     ...
     不过大家不要异想天开地认为如果把这个参数地值设置得更小一点，就能使得系统垃圾收集速度变得更快，
  垃圾收集停顿时间缩短是以牺牲吞吐量和新生代空间为代价换取的：系统把新生代调得小一些，收集300M新生代肯定比收集500M快，
     但这也直接导致垃圾收集发生得更频繁，原来10s收集一次，每次停顿100ms；现在变成5s收集一次，每次停顿70ms。
     停顿时间的确在下降，但吞吐量也降下来了。
     ```
